<h1 align="center">Sarawut Nawawisitkul</h1>
<h3 align="center">Software Engineer · Bangkok, Thailand</h3>

<p align="center">
  <a href="https://github.com/mamochiro">
    <img src="https://komarev.com/ghpvc/?username=mamochiro&style=flat-square&color=0A66C2" alt="Profile Views"/>
  </a>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=58A6FF&center=true&vCenter=true&width=500&lines=%3E_+Building+FinTech+backends+in+Go;%3E_+DDD+%26+Hexagonal+Architecture;%3E_+Microservices+%7C+Kafka+%7C+K8s;%3E_+Systems+that+never+sleep"/>
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=0A66C2&center=true&vCenter=true&width=500&lines=%3E_+Building+FinTech+backends+in+Go;%3E_+DDD+%26+Hexagonal+Architecture;%3E_+Microservices+%7C+Kafka+%7C+K8s;%3E_+Systems+that+never+sleep"/>
  </picture>
</p>

---

## `$ cat about_me.go`

```go
package main

type Engineer struct {
    Name       string
    Role       string
    Location   string
    Experience string
    Languages  []string
    Focus      []string
}

func NewSarawut() Engineer {
    return Engineer{
        Name:       "Sarawut Nawawisitkul",
        Role:       "Software Engineer",
        Location:   "Bangkok, Thailand",
        Experience: "7+ years building mutual fund trading & investment platforms",
        Languages:  []string{"Go", "Scala", "TypeScript", "JavaScript", "PHP"},
        Focus: []string{
            "Golang Microservices & DDD (Hexagonal/Clean Architecture)",
            "Kafka Event-Driven Workflows for Fund Order Processing",
            "Open APIs for Finvest, Odini, FinPlus & Wealth Plus",
            "AI-Assisted SDLC with Claude Code",
        },
    }
}
```

---

## Experience

| Company | Role | Period |
|---------|------|--------|
| **Robowealth Co., Ltd.** | Software Engineer | Mar 2020 – Present |
| **Workmotion Creative** | Software Developer | Jul 2018 – Feb 2020 |
| **Spoon Laboratories Co., Ltd.** | R&D | Aug 2017 – Dec 2017 |

---

## Education

**Prince of Songkla University** — B.Eng. Software Engineering, College of Computing (2014 – 2018)

---

## Tech Stack

### Languages
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/scala/scala-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="45"/>
</p>

### Frontend
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nuxtjs/nuxtjs-original.svg" width="45"/>
</p>

### Backend & APIs
<p>
  <img src="https://img.shields.io/badge/REST-0A66C2?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/gRPC-4285F4?style=for-the-badge&logo=grpc&logoColor=white"/>
  <img src="https://img.shields.io/badge/GraphQL-E10098?style=for-the-badge&logo=graphql&logoColor=white"/>
  <img src="https://img.shields.io/badge/Microservices-4CAF50?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Apache%20Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white"/>
</p>

### Databases
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/elasticsearch/elasticsearch-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg" width="45"/>
</p>

### DevOps & Cloud
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/googlecloud/googlecloud-original.svg" width="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jenkins/jenkins-original.svg" width="45"/>
</p>

<p>
  <img src="https://img.shields.io/badge/GitLab%20CI-FC6D26?style=for-the-badge&logo=gitlab&logoColor=white"/>
  <img src="https://img.shields.io/badge/ArgoCD-EF7B4D?style=for-the-badge&logo=argo&logoColor=white"/>
  <img src="https://img.shields.io/badge/k6-7D64FF?style=for-the-badge&logo=k6&logoColor=white"/>
  <img src="https://img.shields.io/badge/Helm-0F1689?style=for-the-badge&logo=helm&logoColor=white"/>
  <img src="https://img.shields.io/badge/OpenShift-EE0000?style=for-the-badge&logo=redhatopenshift&logoColor=white"/>
  <img src="https://img.shields.io/badge/Traefik-24A1C1?style=for-the-badge&logo=traefikproxy&logoColor=white"/>
  <img src="https://img.shields.io/badge/APISIX-E8433E?style=for-the-badge&logo=apacheapisix&logoColor=white"/>
</p>

---

## Featured Projects

| Project | Description |
|---------|-------------|
| **Trading & Investment Open API** | Financial-grade REST/gRPC APIs with idempotent transaction handling and secure third-party integrations |
| **Wealth Plus (KBank Partnership)** | Cross-team mutual fund platform with performance testing and security collaboration |
| **Financial Data Search Platform** | Elasticsearch optimization for 3,000+ mutual funds with complex nested mappings |
| **Scheduler & Worker Pool Engine** | Concurrent processing for recurring investments using Golang and Kafka |
| **CI/CD & Production Reliability** | GitLab CI pipelines, ArgoCD deployments, and k6 load validation |

---

## GitHub Trophies

<p align="center">
  <img src="https://github-trophies.vercel.app/?username=mamochiro&theme=darkhub&no-frame=true&no-bg=true&column=7"/>
</p>

---

## GitHub Stats

<p align="center">
  <img src="https://github-readme-stats-rickstaa.vercel.app/api?username=mamochiro&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true"/>
  <br/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=mamochiro&theme=tokyonight&hide_border=true"/>
  <br/>
  <img src="https://github-readme-stats-rickstaa.vercel.app/api/top-langs/?username=mamochiro&theme=tokyonight&hide_border=true&layout=compact&langs_count=8"/>
</p>

---

## Activity Graph

<p align="center">
  <img src="https://activity-graph.vercel.app/graph?username=mamochiro&theme=react-dark&hide_border=true&area=true"/>
</p>

---

## Contribution Animation

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/mamochiro/mamochiro/output/github-contribution-grid-snake-dark.svg"/>
    <img src="https://raw.githubusercontent.com/mamochiro/mamochiro/output/github-contribution-grid-snake.svg"/>
  </picture>
</p>

---

<p align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=dark"/>
</p>

---

## Connect with Me

<p align="center">
  <a href="mailto:sarawut.nawawisitkul@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/mamochiro">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://linkedin.com/in/sarawut-nawawisitkul-4431aa181">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>
